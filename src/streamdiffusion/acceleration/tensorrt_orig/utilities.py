...
class Engine:
    ...
    def build(
        ...
        # ============ TACTIC SOURCES ============
        # Tactic sources — version-aware (TRT 11 REMOVED CUBLAS/CUBLAS_LT/CUDNN; same fix as the fork's
        # tensorrt/utilities.py). Build from whatever exists; if none, leave unset (TRT default).
        ...
        # TRT 11 removed the fp16/tf32 BUILDER FLAGS (strongly-typed; ONNX already fp16). Gate on major.
        trt_major = int(trt.__version__.split(".")[0])
        precision_kwargs = {"tf32": True, "fp16": fp16} if trt_major < 11 else {}

        engine = engine_from_network(
            network,
            config=CreateConfig(
                refittable=enable_refit, profiles=[p], load_timing_cache=timing_cache,
                **precision_kwargs, **config_kwargs,
            ),
...
def build_engine(
    ...
    # Enable TF32 for Ampere and newer GPUs (15-25% speedup)
