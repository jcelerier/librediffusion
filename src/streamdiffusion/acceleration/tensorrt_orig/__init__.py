...
def compile_clip(
    ...
    # librediffusion fix: SDXL's encode_prompt uses the PENULTIMATE hidden layer
    # (hidden_states[-2]) for BOTH text encoders' sequence embeddings — NOT last_hidden_state.
    # The old wrapper returned last_hidden_state, so the exported SDXL clip/clip2 engines
    # emitted the wrong layer (golden cos 0.26 vs 1.0 with penultimate) → sdxl conditioning
    # was garbage (psnr 11). Now: penultimate=True picks hidden_states[-2]. SD1.5/SD2.1 keep
    # last_hidden_state (correct for them; penultimate stays False).
    if output_hidden_states:
        # clip2 (SDXL): return (sequence_embeds[B,77,1280], pooled text_embeds[B,1280]).
        ...
        text_encoder_for_export = CLIPWithHiddenStates(text_encoder, penultimate)
    elif penultimate:
        # clip1 (SDXL): the CLIP model spec declares 2 outputs (["text_embeddings","text_embeds"]),
        # matching the raw text_encoder's (last_hidden_state, pooler_output). Keep 2 outputs but
        # swap the FIRST (the sequence embeds the C++ reads) to the PENULTIMATE layer.
        ...
        text_encoder_for_export = CLIPPenultimate(text_encoder)
    else:
...
def accelerate_with_tensorrt(
    ...
    use_cuda_graph: bool = True,  # Enabled by default for 10-15% latency reduction
    engine_build_options: dict = {},
    static_shapes: bool = False,  # False by default for flexibility
    ...
    # Memory pool optimization (2-4% gain)
    ...
    # Optimize CPU threading for GPU workloads (5-10% additional gain)
    ...
    # Apply static shape optimization for better performance
    ...
    if not os.path.exists(clip_engine_path):
        compile_clip(
            ...
            opt_batch_size=opt_batch_size,
            engine_build_options=engine_build_options,
            # SDXL uses the penultimate hidden layer for clip1's sequence embeds too.
            penultimate=stream.sdxl,
        )
    else:
        del text_encoder
    ...
    if stream.sdxl and not os.path.exists(clip2_engine_path):
        ...
        compile_clip(
            ...
            engine_build_options=engine_build_options,
            output_hidden_states=True,  # CRITICAL: Export both outputs!
            penultimate=True,           # SDXL uses hidden_states[-2], not last_hidden_state
        )
    ...
    if not os.path.exists(unet_engine_path):
        if use_prebuilt_onnx:
            ...
            # librediffusion: build_engine() accepts only opt_image_height/width, opt_batch_size, and the
            # four build_* flags — NOT the full engine_build_options dict (which also carries
            # opt_image_height/width + min/max_image_resolution). Forwarding the whole dict on top of the
            # explicit height/width args raised "multiple values for keyword argument 'opt_image_height'".
            # Pull the opt dims from the options (the resolution the caller actually requested; stream.height/
            # width are unset here because no prepare() ran) and pass through only the build_* flags.
            opt_h = engine_build_options.get("opt_image_height", stream.height)
            opt_w = engine_build_options.get("opt_image_width", stream.width)
            build_engine_flags = {
                k: engine_build_options[k]
                for k in ("build_static_batch", "build_dynamic_shape",
                          "build_all_tactics", "build_enable_refit")
                if k in engine_build_options
            }
            build_engine(
                engine_path=unet_engine_path,
                onnx_opt_path=onnx_opt_path,
                model_data=unet_model,
                opt_image_height=opt_h,
                opt_image_width=opt_w,
                opt_batch_size=opt_batch_size,
                **build_engine_flags,
            )
        else:
